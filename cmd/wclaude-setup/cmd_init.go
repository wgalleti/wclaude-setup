...
func runInit() {
	...
	tui.LogInfo("Diretorio: " + config.ClaudeDir())
	fmt.Println()
	...
	// 3. MCPs
	var installMCPs bool
	confirm := huh.NewConfirm().
		Title("Deseja instalar/gerenciar MCPs?").
		Value(&installMCPs)

	if err := huh.NewForm(huh.NewGroup(confirm)).Run(); err == nil && installMCPs {
		runMCPInstall()
	} else {
		tui.LogInfo("MCPs pulados")
	}

	fmt.Println()
	tui.LogSuccess("Setup global finalizado!")
	tui.WaitForEnter()
}
...
func initGlobalCLAUDE() {
	...
	if !setup.GlobalCLAUDEExists() {
		tui.LogInfo("CLAUDE.md nao encontrado, criando...")
	...
	form := huh.NewSelect[string]().
		...
		Options(
			...
			huh.NewOption("Manter como esta", "skip"),
		).
		Value(&action)
...
func initSettings() {
	...
	if !setup.SettingsExists() {
		tui.LogInfo("settings.json nao encontrado, criando...")
	...
	form := huh.NewSelect[string]().
		...
		Options(
			huh.NewOption("Mesclar permissoes (adicionar novas)", "merge"),
	...
	case "merge":
		...
		if added == 0 {
			tui.LogSuccess("Ja esta atualizado, nenhuma permissao nova")
			return
		}
		...
		tui.LogSuccess("settings.json atualizado")
...
// ensureAPIKey carrega config e garante que tem API key.
// Retorna nil se o usuario cancelou.
func ensureAPIKey() (*config.Config, error) {
	cfg, err := config.Load()
	if err != nil {
	...
	if err := huh.NewForm(huh.NewGroup(field)).Run(); err != nil || apiKey == "" {
		...
		return nil, fmt.Errorf("api key nao fornecida")
	}
